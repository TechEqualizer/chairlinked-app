        ...
        <div className="bg-white/90 backdrop-blur-xl border border-gray-200/30 rounded-2xl shadow-lg p-1.5 flex items-center gap-0.5">
          {finalToolbarItems.map((item, index) => (
            <React.Fragment key={item.id}>
              {index === 3 && <div className="w-px h-6 bg-gray-200 mx-1" />}
              {(onAIChat || onQuickEdit) && index >= 6 && index === finalToolbarItems.length - (onQuickEdit ? 1 : 0) && <div className="w-px h-6 bg-gray-200 mx-1" />}
              <motion.button
